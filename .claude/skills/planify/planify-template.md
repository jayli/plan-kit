...
- **文件位置**: `<skill_dir>/plan/` 目录（`skill_dir` 从 planify 这个 skill 根目录里的`settings.json` 读取）
...
1. 读取 `<skill_dir>/settings.json` 中的 `skill_dir` 字段（`skill_dir` 从 planify 这个 skill 根目录`settings.json`读取）
...
3. 从 planify skill 根目录的 `settings.json` 读取 `skill_dir`，计算 plan 文件路径
